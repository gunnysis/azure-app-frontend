...
- **Exception capture (root-cause-fixed 2026-06-25):** `disableExceptionTracking:false` auto-collects `window.onerror` (sync errors) — but that alone does NOT capture **unhandled promise rejections**; per the MS JS-SDK config docs that needs `enableUnhandledPromiseRejectionTracking` which **defaults to `false`**. The earlier code claimed it collected rejections but didn't. It's now set `true` explicitly because the app is async/Promise-heavy (`requestPrediction`, `Promise.all`, image export) and the fallback path *swallows* errors (screen still renders) — so a rejected promise is otherwise invisible. This is the telemetry that makes "fallback opacity" actually diagnosable.
- **All-telemetry enrichment (telemetry initializer):** `appinsights.js` registers an `addTelemetryInitializer` (MS-recommended) stamping every item with `ai.cloud.role` = `jjirit-frontend` (distinguishes the SPA from the separate ML backend in the Application Map) and `ai.application.ver` = `window.SINGLE_ENERGY_APP_VERSION` (set in `config.js`). That populates the `application_Version` column so RUM correlates with releases — a regression's "since which deploy?" becomes a filter, not a guess. **Keep `SINGLE_ENERGY_APP_VERSION` in sync with the `?v=` cache-bust tag in `index.html`** (both currently `20260625-rum-enrich`).
...
- **Where to actually view telemetry (gotcha):** the SWA resource's own *Application Insights* blade in the portal shows *"App Insights is only applicable to Static Web Apps with at least one function…"* and stays disabled — this is **expected** (that blade is for the managed-functions API, which this app lacks), **not** a broken integration. The real RUM lives in the standalone **`appi-frontend-prod-kc-02`** resource (▸ Live metrics for instant verification, ▸ Logs for `union pageViews,customEvents`). **Empty data ≠ broken** — first confirm key/SDK-load/SRI/host-gating are intact (all verified 2026-06-24), then suspect no-traffic + 1–5 min ingestion lag.
- **Azure Activity Log → Log Analytics is NOT set up (permission-blocked).** Activity Log export is **subscription-scoped** (`az monitor diagnostic-settings subscription create`), needing `microsoft.insights/diagnosticSettings/write` at subscription scope. The working account is **Owner on RG `project-1st-team-3` only — no subscription role** → `AuthorizationFailed`. It requires a subscription admin (aisesac) to grant Monitoring Contributor or run it. SWA *resource*-scoped diagnostics (`StaticSiteHttpLogs`/`StaticSiteDiagnosticLogs`/`AllMetrics` → LA) ARE within RG-Owner rights but are server HTTP logs, not the control-plane Activity Log. See `docs/POST_DEPLOY_REVIEW.md` §1-D.
